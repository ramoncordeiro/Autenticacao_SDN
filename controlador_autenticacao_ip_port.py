...
from pox.pox.openflow import of_1
...
class Controlador(object):  
    ...
    def __init__ (self, connection, ip_c, port_c):
    ...
    def send_flagConfirm_to_client(self, m):
        ...
        self._send_packet(ether_pk.pack(),1)
...
def launch (ip_c,port_c):
  ...
  print "IP: ", ip
  print "PORT: ", port
  def start_switch (event,ip,port):
    log.debug("Controlling %s" % (event.connection,))
    Controlador(event.connection,ip,port)
